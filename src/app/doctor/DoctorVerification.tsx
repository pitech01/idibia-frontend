...
export default function DoctorVerification({ onComplete }: DoctorVerificationProps) {
    const [step, setStep] = useState(2); // Start at step 2 as per prompt (Step 1 assumed complete)
    const [folioNumber, setFolioNumber] = useState('');
    const [isVerifyingFolio, setIsVerifyingFolio] = useState(false);
    const [isFolioVerified, setIsFolioVerified] = useState(false);

    // Using onComplete to allow exiting the flow (e.g. Save & Exit)
    const handleExit = () => {
        onComplete();
    };
    ...
    const handleNext = () => {
        ...
        if (step === 3) {
            // Final submission
            setTimeout(() => {
                setStep(4); // Move to pending state
            }, 1000);
        }
    };
    ...
    return (
        <div className="doc-verification-container" style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Outfit', sans-serif" }}>
            ...
            <div style={{ flex: '1.2', background: 'white', padding: '60px 80px', display: 'flex', flexDirection: 'column' }}>

                {/* Header - Progress Stepper */}
                <div style={{ marginBottom: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
                    {/* Step Connectors */}
                    <div style={{ position: 'absolute', top: '15px', left: '0', right: '0', height: '2px', background: '#e2e8f0', zIndex: 0 }}></div>
                    <div style={{ position: 'absolute', top: '15px', left: '0', width: `${(Math.min(step, 4) - 1) * 33}%`, height: '2px', background: '#2563eb', zIndex: 0, transition: 'width 0.3s ease' }}></div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', maxWidth: '600px', margin: '0 auto', width: '100%' }}>
                    ...
                    <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px', marginBottom: '32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <div>
                                <h4 style={{ margin: 0, fontSize: '14px', fontWeight: '600', color: '#0f172a' }}>Dr. Chioma Okeke</h4>
                                <span style={{ fontSize: '12px', color: '#16a34a', display: 'flex', alignItems: 'center', gap: '4px' }}>Verified via NIN <svg width="12" height="12" fill="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></span>
                            </div>
                        </div>
                    ...
                    {step === 2 && (
                        <div className="animate-fade-in">
                            <h2 style={{ fontSize: '28px', fontWeight: 'bold', color: '#0f172a', marginBottom: '8px' }}>Verify your Medical Credentials</h2>
                            <p style={{ color: '#64748b', marginBottom: '32px' }}>We securely cross-reference this information directly with the Medical and Dental Council of Nigeria (MDCN) Registry.</p>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                                {/* Folio Input with Simulation */}
                                <div>
                                    <label style={{ display: 'block', fontSize: '14px', fontWeight: '500', color: '#334155', marginBottom: '8px' }}>MDCN Folio Number</label>
                                    <div style={{ position: 'relative' }}>
                                        <input
                                            ...
                                            style={{
                                                width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '15px', outline: 'none',
                                                borderColor: isFolioVerified ? '#16a34a' : '#cbd5e1'
                                            }}
                                        />
                                        <div style={{ position: 'absolute', right: '16px', top: '14px' }}>
                                            {isVerifyingFolio && <div style={{ color: '#3b82f6' }}><svg className="animate-spin" width="20" height="20" fill="none" viewBox="0 0 24 24"><circle style={{ opacity: 0.25 }} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path style={{ opacity: 0.75 }} fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></div>}
                                            {isFolioVerified && <span style={{ color: '#16a34a' }}><Icons.CheckCircle /></span>}
                                        </div>
                                    </div>
                                    {isFolioVerified && <p style={{ fontSize: '13px', color: '#16a34a', marginTop: '6px', marginLeft: '4px' }}>Valid: Dr. C. Okeke (Registered)</p>}
                                </div>

                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                    <div>
                                        <label style={{ display: 'block', fontSize: '14px', fontWeight: '500', color: '#334155', marginBottom: '8px' }}>Year of Graduation</label>
                                        <input type="text" placeholder="YYYY" style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '15px', outline: 'none' }} />
                                    </div>
                                </div>

                                {/* Drag Drop Zone */}
                                <div>
                                    <label style={{ display: 'block', fontSize: '14px', fontWeight: '500', color: '#334155', marginBottom: '8px' }}>Upload Practicing License</label>
                                    <div style={{ border: '2px dashed #cbd5e1', borderRadius: '16px', padding: '32px', textAlign: 'center', cursor: 'pointer', background: '#f8fafc', transition: 'border 0.2s', color: '#64748b' }}>
                                        ...
                                        <p style={{ fontSize: '14px', fontWeight: '500', color: '#0f172a' }}>Click to upload or drag and drop</p>
                                        <p style={{ fontSize: '12px', marginTop: '4px' }}>PDF or JPG (Max 5MB)</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {step === 3 && (
                        <div className="animate-fade-in">
                            {/* Payout Step - Simplified for demo */}
                            <h2 style={{ fontSize: '28px', fontWeight: 'bold', color: '#0f172a', marginBottom: '8px' }}>Financial Information</h2>
                            <p style={{ color: '#64748b', marginBottom: '32px' }}>Link your bank account for secure weekly payouts.</p>

                            <div style={{ background: '#fff7ed', border: '1px solid #ffedd5', padding: '16px', borderRadius: '12px', marginBottom: '24px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                                <div style={{ color: '#c2410c', marginTop: '2px' }}><svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg></div>
                                <div>
                                    <h4 style={{ fontSize: '14px', fontWeight: '700', color: '#9a3412', margin: 0 }}>Name Match Requirement</h4>
                                    <p style={{ fontSize: '13px', color: '#c2410c', margin: '4px 0 0', lineHeight: '1.4' }}>To prevent fraud, the bank account name MUST match your verified medical license name (Dr. Chioma Okeke).</p>
                                </div>
                            </div>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                <input type="text" placeholder="Bank Name" style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '15px' }} />
                                <input type="text" placeholder="Account Number" style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #cbd5e1', fontSize: '15px' }} />
                                <input type="text" placeholder="Account Name (Auto-fetched)" disabled style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid #e2e8f0', fontSize: '15px', background: '#f1f5f9', color: '#94a3b8' }} />
                            </div>
                        </div>
                    )}
                    ...
                    {step === 4 && (
                        <div className="animate-fade-in" style={{ textAlign: 'center', padding: '40px 0' }}>
                            ...
                            <p style={{ color: '#64748b', fontSize: '16px', lineHeight: '1.6', maxWidth: '400px', margin: '0 auto' }}>
                                Our medical team is currently reviewing your documents. This usually takes <strong>24–48 hours</strong>.
                            </p>
                            <div style={{ marginTop: '32px', padding: '16px', background: '#f8fafc', borderRadius: '8px', fontSize: '14px', color: '#64748b' }}>
                                You will receive an SMS notification at <span style={{ color: '#0f172a', fontWeight: '600' }}>+234 80*** **92</span> once your account is approved.
                            </div>
                        </div>
                    )}

                    {/* Footer Checkbox (Step 2 Only) */}
                    {/* Actions */}
                    {step < 4 && (
                        <div style={{ marginTop: 'auto', paddingTop: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <button
                                onClick={handleNext}
                                disabled={step === 2 && !isFolioVerified}
                                style={{
                                    background: (step === 2 && !isFolioVerified) ? '#cbd5e1' : '#2563eb',
                                    color: 'white', border: 'none', padding: '14px 32px', borderRadius: '12px', fontWeight: '600', fontSize: '16px', cursor: (step === 2 && !isFolioVerified) ? 'not-allowed' : 'pointer',
                                    transition: 'background 0.2s',
                                    boxShadow: (step === 2 && !isFolioVerified) ? 'none' : '0 4px 12px rgba(37, 99, 235, 0.2)'
                                }}
                            >
